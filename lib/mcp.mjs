...
function requestProtocolVersion(rpc, transportMeta) {
  return rpc.params?._meta?.[PROTOCOL_META] ?? transportMeta?.protocolVersion ?? null;
}

function isModernRequest(rpc, transportMeta) {
  const bodyVersion = rpc.params?._meta?.[PROTOCOL_META];
  return rpc.method === "server/discover" || bodyVersion !== undefined || transportMeta?.protocolVersion === MCP_MODERN_PROTOCOL_VERSION;
}

function validateModernRequest(rpc, transportMeta = {}) {
  ...
  if (version !== MCP_MODERN_PROTOCOL_VERSION) {
    return rpcError(rpc.id ?? null, -32022, "Unsupported protocol version", {
      ...
      required: MCP_MODERN_PROTOCOL_VERSION
    });
  }
  ...
  if (!clientCapabilities || typeof clientCapabilities !== "object" || Array.isArray(clientCapabilities)) {
    return rpcError(rpc.id ?? null, -32021, "Missing required client capability metadata", {
      required: CLIENT_CAPS_META
    });
  }
  if (transportMeta.requireHeaders) {
    if (transportMeta.protocolVersion !== version || transportMeta.method !== rpc.method) {
...
export async function handleMcpRpc(body, { fetchImpl = fetch, now = new Date(), transportMeta = {} } = {}) {
  ...
  const modern = isModernRequest(rpc, transportMeta);

  if (modern) {
    const validationError = validateModernRequest(rpc, transportMeta);
    if (validationError) return { status: 400, body: validationError };
    if (rpc.method === "initialize") return { status: 404, body: rpcError(id, -32601, "Method not found") };
  }

  if (rpc.method.startsWith("notifications/")) return { status: 202, body: null };
  if (rpc.method === "ping") return { status: 200, body: rpcResult(id, completeResult({}, modern)) };
