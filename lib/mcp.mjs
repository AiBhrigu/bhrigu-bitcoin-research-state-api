...
export const MCP_MODERN_DISCOVERY_VERSIONS = Object.freeze([MCP_MODERN_PROTOCOL_VERSION]);
...
function validateModernRequest(rpc, transportMeta = {}, classification) {
  ...
  const version = classification.bodyVersion;
  if (version !== MCP_MODERN_PROTOCOL_VERSION) {
    return unsupportedVersion(rpc, version ?? classification.headerVersion ?? "missing");
  }
  ...
  if (rpc.method === "server/discover") {
    return {
      ...
      body: rpcResult(id, completeResult({
        supportedVersions: MCP_MODERN_DISCOVERY_VERSIONS,
