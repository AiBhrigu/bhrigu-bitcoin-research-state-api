...
export function evidenceForWindow(windowId, now = new Date()) {
  return EVIDENCE_LEDGER.filter((item) =>
    item.window_id === windowId && new Date(item.observed_at_utc) <= now
  );
}
...
export function listWindows(now = new Date()) {
  return WINDOWS.map((window) => ({
    ...
    durable_evidence_count: evidenceForWindow(window.id, now).length,
...
export function getWindow(windowId, now = new Date()) {
  ...
  return {
    ...
    evidence: evidenceForWindow(windowId, now)
  };
}
...
export function temporalSummary(now = new Date()) {
  ...
  return {
    ...
    durable_evidence_count: WINDOWS.reduce((sum, window) => sum + evidenceForWindow(window.id, now).length, 0),
