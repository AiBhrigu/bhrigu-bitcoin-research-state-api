...
assert.equal(sha256("../evidence/SEP_17_2026_POSTBOUNDARY.json"), EVIDENCE_INTEGRITY.SEP_17_2026.sha256, "SEP_17_2026 evidence changed");
...
const sep17EvidenceJson = JSON.parse(fs.readFileSync(new URL("../evidence/SEP_17_2026_POSTBOUNDARY.json", import.meta.url), "utf8"));
assert.equal(sep17EvidenceJson.window_id, SEP_17_POSTBOUNDARY_EVIDENCE.window_id);
assert.equal(sep17EvidenceJson.observed_btcusdt, SEP_17_POSTBOUNDARY_EVIDENCE.observation.btcusdt_last_price);
assert.equal(sep17EvidenceJson.runtime_commit, SEP_17_POSTBOUNDARY_EVIDENCE.provenance.runtime_commit);
assert.equal(sep17EvidenceJson.retroactive_rewrite, "forbidden");
...
console.log(JSON.stringify({
  schema: "bhrigu_integrity_tests_v0_3",
  status: "PASS",
  ...
  sep17_evidence_sha256: EVIDENCE_INTEGRITY.SEP_17_2026.sha256,
  checks: 22
}));
